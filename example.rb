...
# AZURE_RESOURCE_LOCATION: your Azure Stack resource location
#
def run_example
  #
  ...
  armEndpoint = ENV['ARM_ENDPOINT']
  storage_endpoint = armEndpoint[armEndpoint.index('.')+1, armEndpoint.length].chop
  ...
  vm = create_vm(compute_client, network_client, LOCAL, 'firstvm', storage_account, vnet.subnets[0], public_ip, storage_endpoint)
...
def create_vm(compute_client, network_client, location, vm_name, storage_acct, subnet, public_ip, storage_endpoint)
